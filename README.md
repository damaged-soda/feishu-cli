...
# 覆盖：Markdown → 已有飞书文档（import 默认就是 replace）
feishu-cli doc import report.md --document-id <document_id>

# 追加：Markdown → 已有飞书文档末尾
feishu-cli doc import report.md --document-id <document_id> --append
...
如果你要用一份新的 Markdown 完整覆盖已有文档内容，可以直接使用 `feishu-cli doc import --document-id ...`，或等价的 `feishu-cli doc replace`。
如果你希望保留旧内容并追加到文档末尾，请显式传入 `--append` 或 `--mode append`。
...
# 覆盖已有文档内容（默认行为）
feishu-cli doc import doc.md --document-id <doc_id>

# 追加到已有文档末尾
feishu-cli doc import doc.md --document-id <doc_id> --append
